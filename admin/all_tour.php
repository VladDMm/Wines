                                ...
                                <div class="table-responsive m-t-40">
                                    <table id="example23" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
                                        ...
                                        <tbody>
                                            ...
                                            <?php
                                            $sql = "SELECT * FROM tourism order by t_id desc";
                                            $query = mysqli_query($db, $sql);

                                            if (!mysqli_num_rows($query) > 0) {
                                                echo '<td colspan="11"><center>Nu sunt tururi</center></td>';
                                            } else {
                                                while ($rows = mysqli_fetch_array($query)) {
                                                    // Obținem service_id din tours
                                                    $mql = "SELECT * FROM tourism WHERE t_id='" . $rows['t_id'] . "'";
                                                    $newquery = mysqli_query($db, $mql);
                                                    $fetch = mysqli_fetch_array($newquery);

                                                    // Obținem title din tabela services pe baza service_id
                                                    $serviceQuery = "SELECT title FROM services WHERE service_id='" . $fetch['service_id'] . "'";
                                                    ...
                                                    echo '<tr>
                                                                        ...
                                                                        <td>
                                                                            <a href="delete_tour.php?menu_del=' . $rows['t_id'] . '" class="btn btn-danger btn-flat btn-addon btn-xs m-b-10">
                                                                                <i class="fa fa-trash-o" style="font-size:16px"></i>
                                                                            </a> 
                                                                            <a href="update_tour.php?menu_upd=' . $rows['t_id'] . '" class="btn btn-info btn-flat btn-addon btn-sm m-b-10 m-l-5">
